...
def SARSA_Q_learning(learning_rate, discount_rate, next_action, agent, pickup_states, dropoff_states):
  ...
  if next_action is None:
    actions_list = getNextActions(agent.state, pickup_states, dropoff_states) # possible Actions in state
    action = getPolicyAction(agent, agent.state, actions_list, pickup_states, dropoff_states) 
  else:
    action = next_action                                                    

  reward = calculateReward(action) # Calculate agent rewards and bank account
  agent.action = action
  agent.updateRewards(reward)
  ...
  new_q_value = old_value + learning_rate * (reward + discount_rate *
                                                                q_table[next_state[0]][next_state[1]][next_state[2]][next_action.value] - old_value)
